...
ARGV.each do |shp|
  ...
  if shp_basename =~ /(.*)\.shp/
    ...
    ShpFile.open(shp) do |shp|
      ...
      #puts puts "arTable = " + arTable.table_name
      
      #go though all the shapes in the file
      shp.each do |shape|
        #puts "shape = " + shape.public_methods.to_s
        puts "shape can " + shape.data.public_methods.to_s
        puts "shape.data.hash ==  " + shape.data.hash.to_s
        shape.data.each {|key, value| puts "#{key} is #{value}" }
        
        #create an ActiveRecord object
        record = arTable.new
        #puts "record = " + record.is_a?(ActiveRecord::Base).to_s\
